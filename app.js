...
function calculateFlightTime(distance, depLat, depLon, arrLat, arrLon) {
   let speed;
   ...
   const JET_STREAM_SPEED = 75;
   let lonDiff = arrLon - depLon;
   if (lonDiff > 180) lonDiff -= 360;
   if (lonDiff < -180) lonDiff += 360;
   const latDiff = arrLat - depLat;
   const ewProportion = distance > 0 ? Math.abs(lonDiff) / (Math.abs(lonDiff) + Math.abs(latDiff) + 0.001) : 0;
   const avgLat = Math.abs((depLat + arrLat) / 2);
   const latFactor = avgLat <= 45 ? avgLat / 45 : (90 - avgLat) / 45;
   const clampedLatFactor = Math.max(0, Math.min(1, latFactor));
   const direction = lonDiff > 0 ? 1 : lonDiff < 0 ? -1 : 0;
   const adjustment = JET_STREAM_SPEED * ewProportion * clampedLatFactor * direction;
   speed = Math.max(speed * 0.5, speed + adjustment);
...
async function handleFlightSubmit(e) {
   ...
   const depLat = parseFloat(departInput.dataset.lat);
   const depLon = parseFloat(departInput.dataset.lon);
   const arrLat = parseFloat(arrivalInput.dataset.lat);
   const arrLon = parseFloat(arrivalInput.dataset.lon);
   const distance = haversineDistance(depLat, depLon, arrLat, arrLon);
   const hours = calculateFlightTime(distance, depLat, depLon, arrLat, arrLon);
   ...
   try {
       if (tripType === 'Round Trip') {
           ...
           const base = { userId: currentUser.uid, type, airline, description, isDeleted: 'N', createdAt: new Date().toISOString() };
           const returnHours = calculateFlightTime(distance, arrLat, arrLon, depLat, depLon);
           await addDoc(collection(db, 'flights'), { ...base, departCode, arrivalCode, date: leg1Date, distance: parseFloat(distance.toFixed(2)), hours: parseFloat(hours.toFixed(2)) });
           await addDoc(collection(db, 'flights'), { ...base, departCode: arrivalCode, arrivalCode: departCode, date: leg2Date, distance: parseFloat(distance.toFixed(2)), hours: parseFloat(returnHours.toFixed(2)) });
       } else {
...
async function handleEditSubmit(e) {
   ...
   const hours = calculateFlightTime(distance, departAirport.lat, departAirport.lon, arrivalAirport.lat, arrivalAirport.lon);
